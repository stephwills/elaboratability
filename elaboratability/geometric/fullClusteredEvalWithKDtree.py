...
def align_cloud_to_vector(ref_attach_coords, ref_adj_coords, cloud_coords, attach_coords, adj_coords):
    ...
    all_coords = [get_rotated_coord(adj_coords, coords, rotat_mat) for coords in all_coords]
    return all_coords
...
def evaluate_with_cloud_coords(mol, h_idx, ligand_coords, ligand_radii,
                               ...
                               prot_coords, prot_radii, prot_ids, tree,
    ...
    ind, dist = tree.query_radius(aligned_coords, r=clash_cutoff, return_distance=True, count_only=False)
    for dec_idx, coord in enumerate(aligned_coords):
        neighbors = ind[dec_idx]
        close_dists = dist[dec_idx]
        if len(neighbors) > 0:
            for neigh, clost_dist in zip(neighbors, close_dists):
                cutoff = 0.63 * (prot_radii[neigh] + cluster_radii[dec_idx])
                if clost_dist < cutoff:
                    n_clashes += 1
                    prot_clash_idxs.append(neigh)
    ...
    if not check_ints:
        ...
        int_ind, int_dist = tree.query_radius(aligned_coords, r=hbond_cutoff, return_distance=True, count_only=False)
        prot_don_idxs = set()
        cluster_acc_idxs = []
        ...
        for dec_idx, coord in enumerate(aligned_coords):
            int_neighbors = int_ind[dec_idx]
            int_dists = int_dist[dec_idx]

            if len(int_neighbors) > 0:
                for neigh, int_dist in zip(int_neighbors, int_dists):
                    if int_dist < hbond_cutoff and prot_is_donor[neigh]:
                        if clust_is_acceptor[dec_idx]:
                            for conf in clust_acceptor_confs[dec_idx]:
                                if conf not in clash_conf_ids:
                                    ...
                                    prot_don_idxs.add(neigh)
                                    break

                    if int_dist < hbond_cutoff and prot_is_acceptor[neigh]:
                        if clust_is_donor[dec_idx]:
                            for conf in clust_donor_confs[dec_idx]:
                                if conf not in clash_conf_ids:
                                    cluster_don_idxs.append(dec_idx)
                                    prot_acc_idxs.add(neigh)
                                    break
...
def evaluate_mol_against_cloud(mol,
    ...
    eval_data = Parallel(n_jobs=cpus, backend="multiprocessing")(
        delayed(evaluate_with_cloud_coords)(mol, h_idx, np.array(ligand_coords), ligand_radii,
                                            cluster_coords, cluster_radii,
                                            cluster_to_conf_dict, conf_to_mol_dict, mol_conf_counts,
                                            pocket_coords, pocket_radii, pocket_ids, tree,
