...
class ReactionAnchorScorer(AnchorScorer):
    ...
    def __init__(self, ligand, pdb_file, cloud, pocket_dist=config.POCKET_DIST, clash_cutoff=config.CLASH_CUTOFF,
        ...
        """
        AnchorScorer.__init__(self, ligand, pdb_file, cloud, pocket_dist, clash_cutoff, hbond_cutoff, cloud_attach_coords,
    ...
    def evaluate(self, vector):
        ...
        if self.check_for_ints:
            ...
            filter_res, filter_feas = filter_multiple_rxns_for_vector_with_aizynthfinder(
                self.ligand, vector.anchor_atom_idx, vector.replace_atom_idxs[0], vector.is_hyd, uniq_smis, self.filter_cutoff, verbose=self.verbose)
    ...
    def binary_scorer(self):
        ...
        for vector_id in self.results:
            ...
            if len(self.results[vector_id]['clashing_mol_ids']) >= self.min_mols_added \
                    and len(self.results[vector_id]['clashing_mol_ids']) <= self.max_mols_added:
                clash_check = True
            else:
                clash_check = False
            ...
            if self.check_for_ints:
                if len(self.results[vector_id]['reacting_prot_ids']) >= self.min_ints_reached:
                    int_check = True
                else:
                    int_check = False
